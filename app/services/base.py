...
class BaseService:
    ...
    async def add_one_and_get_id(self, **kwargs: Any) -> int | str:
        return await getattr(self.uow,
                             self.base_repository).add_one_and_get_id(**kwargs)

    @atomic
    async def add_one_and_get_obj(self, **kwargs: Any) -> Any:
        return await getattr(self.uow,
                             self.base_repository).add_one_and_get_obj(
            **kwargs)

    @atomic
    async def get_by_query_one_or_none(self, **kwargs: Any) -> Any:
        return await getattr(self.uow,
                             self.base_repository).get_by_query_one_or_none(
            **kwargs)
